...
async function RJ() {
  ...
  await driver.get(
    "https://www.jucerja.rj.gov.br/AuxiliaresComercio/Leiloeiros"
  );
    ...
    while (true) {
      ...
      await driver.wait(
        until.elementLocated(
          By.xpath("/html/body/div/section[2]/div/div[1]/section[2]/div[2]/div")
        ),
        10000
      );
      ...
      let base = await driver.findElements(
        By.xpath(
          "/html/body/div/section[2]/div/div[1]/section[2]/div[2]/div/ul//li"
        )
      );
      ...
      for (let i = 0; i < base.length; i++) {
        ...
        nome = html
          .split('<h4 class="u-md-texto u-bold">')[1]
          .split("</h4>")[0]
          .trim();
        if (nome.includes("-")) {
        ...
        matricula = html
          .split(
            '<h6 class="u-md-texto u-semBold u-corCinzaClaro u-inlineBlock">'
          )[1]
          .split("</h6>")[0]
          .trim();

        dataPosse = html
          .split("Data da Posse:</h5>")[1]
          .split("</h6>")[0]
          .trim();
      ...
      await driver.executeScript(
        ...
        await driver.findElement(By.xpath("//li[@class='arrow foward']"))
      );

      // se a <div id="loading" class=""> aparecer, espera ela sumir para poder clicar no botão
      await driver.wait(
        until.elementLocated(By.xpath("//div[@id='loading']")),
        10000
      );

      await driver.wait(
        until.elementIsNotVisible(
          await driver.findElement(By.xpath("//div[@id='loading']"))
        ),
        10000000000
      );
